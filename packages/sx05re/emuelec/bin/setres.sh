...
set_main_framebuffer() {
  ...
  if [[ -n "${FBW}" && "${FBW}" > 0 && -n "${FBH}" && "${FBH}" > 0 ]]; then
    ...
    echo 0 0 $(( FBW-1 )) $(( FBH-1 )) > /sys/class/graphics/fb$max_fb/free_scale_axis
    echo 0 > /sys/class/graphics/fb$max_fb/free_scale
    echo 0 > /sys/class/graphics/fb$max_fb/freescale_mode
  fi
}
